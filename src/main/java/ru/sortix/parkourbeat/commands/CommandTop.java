// ФАЙЛ: src/main/java/ru/sortix/parkourbeat/commands/CommandTop.java
...
public class CommandTop {
    ...
    public void onCommand(@Context Player sender, @Arg("settings-players-all") Optional<GameSettings> gameSettingsOpt) {
        ...
        if (gameSettingsOpt.isEmpty()) {
            new GlobalStatisticsMenu(this.plugin, lang, sender).open(sender);
            return;
        }
        ...
        new LevelTopMenu(this.plugin, lang, settings, sender).open(sender);
    }
}
